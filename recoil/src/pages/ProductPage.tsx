...
import { checkoutState, setCheckoutId } from "../recoil/checkout";
...
const ProductPage: React.FC = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      if (checkout.checkoutId) {
        await addLineToCheckout(checkout.checkoutId);
      } else {
  ...
  const addLineToCheckout = async (checkoutId: string) => {
    ...
    if (linesAddData?.checkoutLinesAdd?.errors?.length) {
      ...
      setCheckout((prevState) => ({
        ...prevState,
        checkoutId: setCheckoutId(linesAddData.checkoutLinesAdd!.checkout!.id),
      }));
    }
  };
  ...
  const createNewCheckout = async () => {
    ...
    if (newCheckoutData?.checkoutCreate?.errors?.length) {
      ...
      setCheckout((prevState) => ({
        ...prevState,
        checkoutId: setCheckoutId(newCheckoutData.checkoutCreate!.checkout!.id),
      }));
    }
  };
