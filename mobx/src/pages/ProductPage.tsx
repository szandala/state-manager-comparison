...
export const ProductPage: React.FC = observer(() => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const checkoutId = checkoutStore.checkoutId;
      if (checkoutId) {
  ...
  const addLineToCheckout = async (checkoutId: string) => {
    ...
    if (linesAddData?.checkoutLinesAdd?.errors?.length) {
      ...
      checkoutStore.setCheckoutId(linesAddData.checkoutLinesAdd.checkout.id);
      checkoutStore.saveCheckoutId();
    }
  };
  ...
  const createNewCheckout = async () => {
    ...
    if (newCheckoutData?.checkoutCreate?.errors?.length) {
      ...
      alert("Successfully added to cart", "success");
      checkoutStore.setCheckoutId(newCheckoutData.checkoutCreate.checkout.id);
      checkoutStore.saveCheckoutId();
    }
  };
